...
<div class="login-wrap">
  <div class="login-card">
    ...
    <div class="login-body">
      ...
      {{-- Flash messages --}}
      @if(session('success'))
        <x-alert type="success" style="margin-bottom:16px">{{ session('success') }}</x-alert>
      @endif
      @if(session('error'))
        <x-alert type="error" style="margin-bottom:16px">{{ session('error') }}</x-alert>
      @endif
      @if(session('info'))
        <x-alert type="info" style="margin-bottom:16px">{{ session('info') }}</x-alert>
      @endif
    ...
    <div class="login-footer">
      Protected by ExpenseTracker Security · {{ now()->year }}
    </div>
