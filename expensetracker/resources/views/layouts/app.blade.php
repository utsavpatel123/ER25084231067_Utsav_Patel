...
<title>@yield('title','Dashboard') — ExpenseTracker</title>
  ...
  <aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-footer">
      <div class="sidebar-user">
        <div class="user-avatar">{{ strtoupper(substr(session('admin_username','A'),0,1)) }}</div>
        <div style="flex:1;min-width:0">
          <div class="user-name">{{ session('admin_name','Administrator') }}</div>
          <div class="user-role">{{ session('admin_username') }}</div>
        </div>
      ...
      @foreach(['success','error','info','warning'] as $type)
        @if(session($type))
          <div class="alert alert-{{ $type }}" role="alert">
            <span class="alert-text">{{ session($type) }}</span>
